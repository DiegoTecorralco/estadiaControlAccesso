...
export const Table2grade = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const filtered = data.data.filter((student: Student) => student.grade === 2);
  ...
  const renderTable = (grupo: string, data: Student[]) => (
    <div className="mb-5">
      ...
      <p>Aquí están todos los estudiantes inscritos en el 2° grado grupo {grupo}</p>
          ...
          <tbody>
            {data.map((student) => (
              <tr key={student.nia}>
                ...
                <td>{student.group}</td>
                <td>
                  <button className="btn btn-success btn-sm" onClick={() => handleEdit(student)}>Editar</button>
                </td>
                <td>
                  <button className="btn btn-danger btn-sm" onClick={() => handleDeleteClick(student)}>Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
